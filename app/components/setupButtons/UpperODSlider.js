...
class UpperODSlider extends React.Component {
  ...
  clickSubmit = (event) => {
      this.props.onSubmitButton("upper", (Math.round(this.state.value * 100)/100).toFixed(2));
  }
            ...
            <button type="button" className="btn btn-outline-secondary btn-circle setupButton" onClick={this.clickSubtract}><i className="fa fa-minus"></i></button>
              <button className = "btn btn-lg btn-outline-secondary setupButton" onClick={this.clickSubmit}>
                <span className="slider-label">
                   Upper OD: {(Math.round(this.state.value * 100)/100).toFixed(2)}
                </span>
              </button>
...
UpperODSlider.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(UpperODSlider);
